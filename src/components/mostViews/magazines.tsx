...
const magazineMostViews = async () => {
  ...
  return (
    <div className="w-full h-full ">
      ...
      {data.length > 0 ? (
         <div className="w-full  grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 py-5">
          {data.map((magazine: Magazine, index: number) => (
            <div
              className="w-full h-full flex flex-col gap-1 shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px]   relative"
              key={index}
            >
              <img
                src={magazine.cover}
