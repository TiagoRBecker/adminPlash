...
const EditSponsor = ({ params }: { params: { id: string } }) => {
  ...
  const {data:session, status} = useSession()
    ...
    const getSponsor = async ()=>{
        ...
        const token = session?.user.token 
          const sponsor = await fetch(`${baseURL}sponsor/${params.id}`, {
            method: "GET",
            cache:"no-cache",
            headers:{
          ...
          const response = await sponsor.json()
    ...
    const onSubmit = handleSubmit(async (data) => {
      ...
      const addEvent = await Swal.fire({
        position: "center",
        title: "Tem certeza?",
        text: `Você deseja editar o patrocinador  ${data.name}?`,
      ...
      if (addEvent.isConfirmed) {
        try {
          ...
          //@ts-ignore
          const token = session?.user.token 
          const addArticle = await fetch(`${baseURL}sponsor/edit/${params.id}`, {
            method: "POST",
            headers:{
          ...
          await Swal.fire(
            "Patrocinador editado com sucesso!!",
          ...
          <>
            {cover || newCover ? (
              <div className="w-full h-full flex items-center justify-center relative">
                <img
                  src={cover ?  cover :URL.createObjectURL(newCover as any)}
                  alt=""
